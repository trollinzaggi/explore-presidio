...
Removes bias from Experience, Education, Qualification, and Affiliation sections
"""
...
def anonymize_profile(profile_data):
    """
    Function to anonymize a profile focusing on experience, education, qualification, and affiliation sections.
    ...
    bias_removal_sections = ['experience', 'qualification', 'education', 'affiliation']
    ...
    def process_value(value, field_name="", parent_path=""):
        ...
        if isinstance(value, dict):
            ...
            # Remove organization/institution names if in those fields AND in bias sections
            if in_bias_section and field_name in org_name_fields:
                ...
                if any(name in value.lower() for name in ['university', 'college', 'institute', 'school', 'company', 'corp', 'inc', 'ltd', 'club', 'alliance', 'foundation', 'association']):
                    return "removed"
            
            # If we're in experience/qualification/education/affiliation section, remove bias
            if in_bias_section:
...
def main():
    ...
    with st.sidebar:
        ...
        st.subheader("🎯 Anonymization Scope")
        st.info("""
        **Sections Processed:**
        - ✅ Experience
        - ✅ Education
        ...
        if selected_entry:
            ...
            with col2:
                if st.button("🔒 Anonymize Profile", type="primary", use_container_width=True):
                    with st.spinner("Removing bias from Experience, Education, Qualification & Affiliation sections..."):
                ...
                if st.session_state.selected_profile and st.session_state.anonymized_profile:
                    ...
                    if st.button("📊 Show Changes", use_container_width=True):
                        with st.expander("What Changed", expanded=True):
                            ...
                            with col1:
                                st.write("**✅ Anonymized Sections:**")
                                st.write("")
                                st.write("**Experience:**")
                                st.write("- Removed bias from job titles")
                                st.write("- Removed bias from descriptions")
                                st.write("- Company names → 'removed'")
                                st.write("")
                                st.write("**Education/Qualification:**")
                                st.write("- Institution names → 'removed'")
                                st.write("- Removed bias from achievements")
                                st.write("- Kept degrees and years")
                            
                            with col2:
                                st.write("**✅ Affiliation Changes:**")
                                st.write("")
                                st.write("**Boards:**")
                                st.write("- Organization names → 'removed'")
                                st.write("- Removed bias from positions")
                                st.write("")
                                st.write("**Awards/Mandates:**")
                                st.write("- Removed bias from names")
                                st.write("- Removed bias from descriptions")
                                st.write("")
                                st.write("**Memberships:**")
                                st.write("- Organization names → 'removed'")
                                st.write("- Removed bias from roles")
    ...
    st.markdown("""
        <div style='text-align: center; color: #888; padding: 20px;'>
            <p>Focused Anonymization: Experience, Education, Qualification & Affiliation</p>
        </div>
    """, unsafe_allow_html=True)
